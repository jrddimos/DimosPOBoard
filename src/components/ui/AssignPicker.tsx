...
// Variante multi-sélection — réservée aux US (une sous-tâche garde un seul
// assigné, cf. AssignPicker ci-dessus). `value`/`onChange` manipulent une
// liste de trigrammes ; le stockage reste la même colonne texte
// (parseAssignees/serializeAssignees, src/lib/utils.ts), juste avec
// plusieurs trigrammes séparés par des virgules.
export function AssignPickerMulti({ value, membres, onChange, disabled }: {
  value: string[]; membres: UserProfile[]
  onChange: (list: string[]) => void; disabled?: boolean
}) {
  const [open, setOpen] = useState(false)
  const ref = useRef<HTMLDivElement>(null)
  useClickOutside(ref, () => setOpen(false), open)
  ...
  function toggle(tri: string) {
    onChange(value.includes(tri) ? value.filter(x => x !== tri) : [...value, tri])
  }

  if (disabled) return value.length
    ? <span className="flex items-center gap-1 flex-wrap">
        {value.map(tri => <span key={tri} className="text-xs px-1.5 py-0.5 rounded-full bg-indigo-50 text-indigo-700 font-medium">{tri}</span>)}
      </span>
    : null
  ...
  return (
    <div className="relative inline-block" ref={ref} onClick={e => e.stopPropagation()}>
      <div className="flex items-center gap-1 flex-wrap">
        {value.map(tri => (
          <div key={tri} className="flex items-center gap-1">
            <button type="button" onClick={() => setOpen(o => !o)}
              className="text-xs px-1.5 py-0.5 rounded-full bg-indigo-50 text-indigo-700 font-medium hover:bg-indigo-100 transition-colors">
              {tri}
            </button>
            <button type="button" onClick={() => onChange(value.filter(x => x !== tri))}
              className="w-3.5 h-3.5 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-rose-50 hover:text-rose-500 transition-colors">
              <X size={8} />
            </button>
          </div>
        ...
        <button type="button" onClick={() => setOpen(o => !o)}
          className="flex items-center gap-1 text-xs px-1.5 py-0.5 rounded-full border border-dashed border-slate-200 text-slate-400 hover:border-indigo-300 hover:text-indigo-600 transition-colors">
          <Plus size={10} /> {value.length ? '' : 'Assigner'}
        </button>
      </div>
      {open && (
        <div className="absolute left-0 top-full mt-1 z-50 bg-card border border-slate-200 rounded-xl shadow-lg py-1 min-w-[180px] max-h-64 overflow-y-auto">
          {actifs.map(m => {
            const sel = value.includes(m.trigramme!)
            return (
              <button type="button" key={m.user_id} onClick={() => toggle(m.trigramme!)}
                className={cn('w-full flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-slate-50 text-navy text-left transition-colors',
                  sel && 'font-semibold text-indigo-600 bg-indigo-50/50')}>
                <span className="w-5 h-5 rounded-full bg-indigo-50 text-indigo-700 font-bold flex items-center justify-center text-[10px] shrink-0">
                  {m.trigramme}
                </span>
