...
import { ChevronDown, X, Plus, Zap, CalendarDays, AlertTriangle } from 'lucide-react'
        ...
        <div className="absolute left-0 top-full mt-1 z-50 bg-white border border-slate-200 rounded-xl shadow-lg py-1 min-w-[160px]">
          {actifs.filter(m => m.trigramme).map(m => (
            <button
              ...
              className={cn(
                'w-full flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-slate-50 text-navy text-left transition-colors',
          ...
          {([
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={cn('flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-semibold transition-all',
                activeTab === tab.key ? 'bg-white shadow-sm text-navy' : 'text-slate-400 hover:text-navy')}>
              {tab.icon} {tab.label}
            </button>
          ))}
        </div>
            ...
            {COLS.map(col => {
              ...
              return (
                <div key={col.key} className="kanban-col">
                  ...
                  <div className={cn('flex items-center justify-between pb-2 border-b border-slate-200/70 mb-1 -mx-3 px-3 -mt-3 pt-3 mb-3', col.headerBg)}>
                    ...
                    return (
                      <div key={t.id_tache}
                        className={cn(
                          ...
                          col.borderColor,
                          panel?.id_tache === t.id_tache && 'selected'
                        )}
                        ...
                        {!isReadOnly && (
                          <div className="relative">
                            ...
                            {blockedByTask && (
                              <div className="flex items-center gap-1 mt-1 mb-1 px-2 py-1 rounded-lg bg-amber-50 border border-amber-200 text-amber-600 text-[10px] font-medium">
                                ...
                                {subs.map(s => (
                                  <div key={s.id_tache} className="flex flex-col gap-1">
                                    <label className="flex items-start gap-2 cursor-pointer" onClick={e => e.stopPropagation()}>
                                      {!isReadOnly && (
                                        <input type="checkbox" checked={s.statut === 'Fait'} onChange={() => toggleSub(s)}
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        {s.assigne_a && (
                                          <span className="ml-1 text-xs bg-indigo-50 text-indigo-700 px-1.5 rounded-full">{s.assigne_a}</span>
                                        )}
                  ...
                  {!colTaches.length && (
                    <div className="flex flex-col items-center justify-center h-16 border-2 border-dashed border-slate-200 rounded-xl text-slate-300 text-xs gap-1">Vide</div>
                  )}
                </div>
              )
            })}
          ...
          <div className="w-72 shrink-0 animate-in">
            <div className="ds-card sticky top-0 flex flex-col gap-3 max-h-[calc(100vh-120px)] overflow-y-auto">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setPanel(null)} className="p-1 rounded-lg hover:bg-slate-50 text-slate-400 hover:text-navy">
                  <X size={13} />
                </button>
              </div>
              ...
              {panel.description && (
                <div><div className="ds-label mb-1">User Story</div><p className="text-xs text-navy leading-relaxed whitespace-pre-line">{panel.description}</p></div>
              )}
              {panel.criteres && (
                <div><div className="ds-label mb-1">Critères</div><p className="text-xs text-navy leading-relaxed whitespace-pre-line">{panel.criteres}</p></div>
              )}
              {panel.lien_dod && (
                <div><div className="ds-label mb-1">Lien DoD</div><span className="text-xs text-indigo-600 font-medium">{panel.lien_dod}</span></div>
              )}
              {panel.commentaire && (
                <div><div className="ds-label mb-1">Commentaire</div><p className="text-xs text-slate-400 italic">{panel.commentaire}</p></div>
              )}
                  ...
                  {(childMap[panel.id_tache] ?? []).map(s => (
                    <div key={s.id_tache} className="flex items-center gap-2 py-1">
                      <div className={cn('w-2 h-2 rounded-full shrink-0',
                        ...
                        s.statut === 'Bloqué'   ? 'bg-rose-400' :
                        'bg-slate-300')} />
                      <span className={cn('text-xs flex-1', s.statut === 'Fait' && 'line-through text-slate-400')}>{s.titre}</span>
                      {s.assigne_a && (
                        <span className="text-xs bg-indigo-50 text-indigo-700 px-1.5 rounded-full">{s.assigne_a}</span>
                      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-80 p-5 space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-navy">
                Jours réalisés <span className="text-rose-500">*</span>
              </label>
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={confirmEffort} disabled={effortInput === ''}
