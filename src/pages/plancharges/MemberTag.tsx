...
export function MemberTag({ profile }: { profile: UserProfile }) {
  ...
  return (
    <span className="inline-flex items-center gap-1">
      <span className="inline-flex items-center justify-center w-5 h-5 rounded-full text-white text-[10px] font-bold shrink-0"
        style={{ background: profile.couleur ?? '#4A4CC8' }}>
        {initials.slice(0,2)}
      </span>
      <span className="text-[11px] text-navy/70 font-medium truncate max-w-[100px]">
