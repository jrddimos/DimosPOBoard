...
function StepJalons() {
  ...
  async function add() {
    ...
    await createJalon.mutateAsync({ code: c, couleur })
    setCode('')
  }

  return (
    <div className="flex flex-col gap-4">
      <p className="text-xs text-subtle">Les incréments machine attendus (ex: I1, I2 — proto A, proto B…).</p>
      <Repeater items={jalons} onDelete={id => deleteJalon.mutate(id)} deleting={deleteJalon.isPending}
        renderItem={j => <span className="font-mono font-bold text-sm text-navy">{j.code}</span>}>
        <div className="flex items-end gap-2">
          <div className="flex-1"><label className="ds-label mb-1 block">Code</label>
            <input value={code} onChange={ev => setCode(ev.target.value.toUpperCase())} className="ds-input w-32" maxLength={5} placeholder="I1" onKeyDown={ev => ev.key === 'Enter' && add()} /></div>
          <button onClick={add} disabled={createJalon.isPending || !code.trim()} className="ds-btn-primary ds-btn-sm flex items-center gap-1"><Plus size={13} /> Ajouter</button>
        </div>
      </Repeater>
    </div>
  )
